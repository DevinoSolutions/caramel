...
test.describe('Profile Page', () => {
    ...
    async function checkAuthAndSkip(page) {
    ...
    test.describe('Profile Display - Requires Authentication', () => {
        test.fixme('should display profile header with user info', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
            ...
            const banner = page.locator('.from-caramel, .to-orange-400').first()
        ...
        test.fixme('should display Account Information section', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
        ...
        test.fixme('should show Edit Profile button when not editing', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
    ...
    test.describe('Profile Editing - Requires Authentication', () => {
        test.fixme('should enter edit mode when clicking Edit Profile', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
        ...
        test.fixme('should update profile name successfully', async ({ page }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
                test.skip()
                return
        ...
        test.fixme('should cancel edit and restore original values', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
        ...
        test.fixme('should disable save button when loading', async ({ page }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
                test.skip()
                return
    ...
    test.describe('Profile Status Indicators', () => {
        ...
        test.fixme('should display verified badge for verified emails', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
    ...
    test.describe('Sign Out Functionality - Requires Authentication', () => {
        test.fixme('should sign out when clicking Sign Out button', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
        ...
        test.fixme('should show login buttons after sign out', async ({ page }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
                test.skip()
                return
    ...
    test.describe('Profile Avatar Display - Requires Authentication', () => {
        test.fixme('should display user initial when no image available', async ({
            page,
        }) => {
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
    ...
    test.describe('Mobile Responsiveness - Requires Authentication', () => {
        test.fixme('should display profile correctly on mobile', async ({
            page,
        }) => {
        ...
        test.fixme('should have working buttons on mobile', async ({ page }) => {
            await page.setViewportSize({ width: 375, height: 667 })
            const isAuth = await checkAuthAndSkip(page)
            if (!isAuth) {
                test.skip()
