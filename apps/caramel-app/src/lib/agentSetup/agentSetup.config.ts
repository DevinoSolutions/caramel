...
- \`GET ${API.coupons}?site=<store domain>&limit=<n>\` returns coupon codes for a store (limit defaults to 10, caps at 50; \`page=<n>\` pages). Each \`coupons[]\` object carries \`id\`, \`code\`, \`site\`, \`title\`, \`description\`, \`rating\`, \`discount_type\` (uppercase, e.g. \`PERCENTAGE\`, or null), \`discount_amount\` (number or null), \`expiry\` (an opaque display string such as \`"-"\`, or null; do not parse it as a date), \`expired\`, \`timesUsed\`, \`status\` (the latest verification result, e.g. \`valid\`, \`pending\`, \`retry\`, \`invalid\`), \`verificationMessage\` (the store's own sentence explaining a restricted coupon, otherwise null) and \`lastWorkedAt\` (ISO time or null). \`total\`, \`page\`, \`limit\` and \`hasMore\` describe the page.
