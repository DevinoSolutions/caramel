        ...
        const earlySuccessPromise = new Promise((resolve) => {
            earlyCheckInterval = setInterval(() => {
                ...
                if (!isNaN(currentPrice) && currentPrice < original) {
                    ...
                    resolve('early-success')
                }
            }, 200)
        })
        waiters.push(earlySuccessPromise)

        const via = await Promise.race(waiters)
        if (earlyCheckInterval) clearInterval(earlyCheckInterval)
...
async function fetchCoupons(site, kw) {
    ...
    try {
        ...
        log('fetchCoupons failed:', response?.error || 'Unknown error')
        return []
    } catch (e) {
...
async function startApplyingCoupons(rec) {
    ...
    for (let i = 0; i < coupons.length; i++) {
        ...
        const perf = sitePerformance.get(domain)
        const pauseTime = perf && perf.avgResponseTime < 800 ? 60 : 120
